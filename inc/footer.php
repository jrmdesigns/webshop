    <footer id="page-footer">
        <div id="footer-menu">
            <ul>
                <li>
                    <h1>FEATURED</h1>
                </li>
                <a href="#">
                    <li>LINK</li>
                </a>
                <a href="#">
                    <li>LINK</li>
                </a>
                <a href="#">
                    <li>LINK</li>
                </a>
                <a href="#">
                    <li>LINK</li>
                </a>
               
            </ul>

            <ul>
                <li>
                    <h1>INFO</h1>
                </li>
                <a href="#">
                    <li>LINK</li>
                </a>
                <a href="#">
                    <li>LINK</li>
                </a>
                <a href="#">
                    <li>LINK</li>
                </a>
                <a href="#">
                    <li>LINK</li>
                    
                </a>
            </ul>
        </div>
        <div id="social-media">

            <h1>WEBSHOP</h1>
            <div id="social-media-wrapper">
                <a href="https://www.facebook.com" target="_blank"><span class="fab fa-facebook"></span></a> 
                <a href="https://www.twitter.com/" target="_blank"><span class="fab fa-twitter-square"></span></a>
                <a href="https://www.linkedin.com/uas/login" target="_blank"><span class="fab fa-linkedin"></span></a>
            </div>
        </div>

        <div id="footer-copyright">
           <span>&copy; Copyright Gorilla webshop</span>
           <a href="https://www.github.com/jrmdesigns/" target="_blank">designed by jrmdesigns</a>
        </div>
    </footer>
</section>
</body>
</html>